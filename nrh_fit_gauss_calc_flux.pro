function my2Dgauss, x, y, pars
	...
	; pars[0] Background flux
	; pars[1] Amplitude of Gaussian
	; pars[2] Width in rotated X direction
	; pars[3] Width in rotated Y direction
	; pars[4] X mean
	; pars[5] Y mean
	; pars[6] Roation angle
...
pro nrh_fit_gauss_calc_flux

	; From nrh_choose_centroid.pro, but better name here.
	...
	winsize=600
	window, 0, xs=winsize, ys=winsize, retain=2, xpos=1900, ypos=900
	window, 1, xs=winsize, ys=winsize, retain=2, xpos=1900, ypos=300
	winsize=500
	window, 2, xs=winsize, ys=winsize, xpos = 1300, ypos=900, retain=2
	window, 3, xs=winsize, ys=winsize, xpos = 1300, ypos=700, retain=2
	window, 4, xs=winsize, ys=winsize, xpos = 1300, ypos=500, retain=2
	window, 5, xs=winsize, ys=winsize, xpos = 1300, ypos=300, retain=2
	!p.charsize=1.5
	
	tstart = anytim(file2time('20140901_110230'), /utim)	;anytim(file2time('20140418_125000'), /utim)	;anytim(file2time('20140418_125546'), /utim)	;anytim(file2time('20140418_125310'), /utim)
	...
	while tstart lt tstop do begin
		...
		junk_array[*] = 6.0	; The backround logTb value. 6 for above 228 MHz
		junk_array[ junk_arr_halfsz:junk_arr_halfsz+x_len-1, $
					junk_arr_halfsz:junk_arr_halfsz+y_len-1] = alog10(source_section) > 6.0	
