pro oplot_nrh_on_three_color_20140901, tstart, freq_tags=freq_tags, back_sub=back_sub
	...
	colors = reverse([2, 7, 10])	;reverse((indgen(9)+2.))	;reverse([6,8,4,10])	;[2, 3, 7, 10];(indgen(9)+2.)
					;colors = [6, 7, 10]
	inds = reverse([0, 5, 7])		;[0, 1, 5, 7] ;indgen(9)
	for j=0, n_elements(inds)-1 do begin
		...
		data_roi = nrh_data[0:32, 64:127] 	; For determinging source max for 2014-09-01 event
		max_val = max( (data_roi) ,/nan) 							   
		nlevels=7.0   

		top_percent = 0.40	; 0.3 on a linear scale for the 2014 Sep 01 event
		...
		xyouts, 0.2, 0.76, 'NRH '+anytim(nrh_hdr.date_obs, /cc, /trun)+' UT', /normal, charsize=1.5, color=1 
		...
		if keyword_set(freq_tags) then begin
			if j eq n_elements(inds)-1 then begin
				xyouts, xpos_nrh_lab+0.0021, ypos_nrh_lab - (j)/38.0, 'NRH '+freq_tag+' MHz '+anytim(nrh_hdr.date_obs, /cc, /trun)+' UT', color=1, /normal
				xyouts, xpos_nrh_lab-0.0021, ypos_nrh_lab - (j)/38.0, 'NRH '+freq_tag+' MHz '+anytim(nrh_hdr.date_obs, /cc, /trun)+' UT', color=1, /normal
