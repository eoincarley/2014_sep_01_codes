pro aia_dt_arc_calc_20140901
	...
	winsz=700
	AU = 1.49e11	; meters
	aia_waves = ['171', '211', '193'] ;['171A', '211A', '193A']	;['094A', '131A', '335A']	
	min_exp = [1.5, 1.5, 1.0]
	npoints = 1000
	angle0=70.0
	angle1=270.0
	...
	arc1 = transpose( [ [(radius+120.0)*cos(angles*!dtor)], [(radius+120.0)*sin(angles*!dtor)] ] )
	arc2 = transpose( [ [(radius+140.0)*cos(angles*!dtor)], [(radius+140.0)*sin(angles*!dtor)] ] )	;808.0
	arc3 = transpose( [ [(radius+160.0)*cos(angles*!dtor)], [(radius+160.0)*sin(angles*!dtor)] ] )
	...
	for k=0, n_elements(aia_waves)-1 do begin
		...
		;
		folder = '~/Data/2014_sep_01/sdo/'+aia_waves[k]+'/'
		...
		start = closest(anytim(ind.date_obs, /utim), anytim('2014-09-01T10:55:00', /utim))
		finish = closest(anytim(ind.date_obs, /utim), anytim('2014-09-01T11:15:00', /utim))
		...
		dt_map_struct = {name:'dt_map_'+WAVEL, dtmap:distt, time:tarr, distance:lindMm, xyarcsec:[[arc0], [arc1], [arc2], [arc3]] }
	  	save, dt_map_struct, $
	  		filename='~/Data/2014_sep_01/sdo/dist_time/aia_'+WAVEL+'arc_dt_map_2.sav'
  	ENDFOR		
